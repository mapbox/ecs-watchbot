...
test('getTagForSha: commit found', async (assert) => {
  const execStub = sinon.stub(wbg, 'exec').callsFake((command) => {
    assert.equals(
      command,
      'git ls-remote --tags https://github.com/mapbox/ecs-watchbot'
    );
    return Promise.resolve({
      stdout: 'f4815eb9f3bcfba88930bbe12d0888254af7cfa6\t/refs/tags/v4.1.1'
    });
  });
...
test('uploadBundle: tag found (Tag created using `npm version <patch|minor|major>`) for all except alpine', async (assert) => {
  process.env.CODEBUILD_RESOLVED_SOURCE_VERSION =
    'f4815eb9f3bcfba88930bbe12d0888254af7cfa6';
  ...
  const fsCreateReadStreamStub = sinon
  ...
  assert.equals(
    execStub.args[0][0],
    'npm ci --production',
    'reinstalled npm modules'
  );
  assert.equals(
    execStub.args[1][0],
    'npm install -g pkg',
    'globally installed pkg'
  );
  assert.equals(
    execStub.args[2][0],
    'pkg --targets node18-linux,node18-macos,node18-win .',
    'ran expected pkg command'
  );
  assert.equals(
    execStub.args[3][0],
    'git ls-remote --tags https://github.com/mapbox/ecs-watchbot',
    'listed tags on github'
  );
  ...
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/linux/v4.1.1/watchbot'
    ),
    'logged upload of linux binary'
  );
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/macosx/v4.1.1/watchbot'
    ),
    'logged upload of macos binary'
  );
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/windows/v4.1.1/watchbot'
    ),
    'logged upload of win binary'
  );
...
test('uploadBundle: tag found (Tag created using `npm version <patch|minor|major>`) for alpine', async (assert) => {
  ...
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/alpine/v4.1.1/watchbot'
    ),
    'logged upload of alpine binary'
  );
...
test('uploadBundle: tag found (Tag created manually) for all except alpine', async (assert) => {
  ...
  const execStub = sinon.stub(wbg, 'exec').callsFake(() => {
    return Promise.resolve({
      stdout: 'f4815eb9f3bcfba88930bbe12d0888254af7cfa6\t/refs/tags/4.1.1'
    });
  });
  const fsCreateReadStreamStub = sinon
    .stub(fs, 'createReadStream')
  ...
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/linux/4.1.1/watchbot'
    ),
    'logged upload of linux binary'
  );
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/macosx/4.1.1/watchbot'
    ),
    'logged upload of macos binary'
  );
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/windows/4.1.1/watchbot'
    ),
    'logged upload of win binary'
  );
...
test('uploadBundle: tag found (Tag created manually) for alpine', async (assert) => {
  ...
  assert.ok(
    log.calledWith(
      'Uploading the package to s3://watchbot-binaries/alpine/4.1.1/watchbot'
    ),
    'logged upload of alpine binary'
  );
  fsCreateReadStreamStub.restore();
