...
**fargate** | If a `fargate` object is provided, the Watchbot containers will run on Fargate capacity. | Object/null | No | `null`
**fargate.spot** | Use Fargate Spot instances. This is done by setting a capacity provider strategy on the Watchbot ECS service, so the ECS cluster that you use must be configured to allow services to use the `FARGATE_SPOT` capacity provider. | Boolean/Ref | No | false
**fargate.securityGroups** | The IDs of security groups to associate with the Watchbot container. If you do not specify a security group, the default security group for the VPC is used. You can provide no more than 5. | String[]/Ref | No | []
**fargate.subnets** | The IDs of subnets to associate with the Watchbot container. If you do not specify a security group, the default security group for the VPC is used. You can provide no more than 16. | String[]/Ref | No | []
...
**reservation.memory** | The number of MB of RAM to reserve as a hard limit. If your worker container tries to utilize more than this much RAM, it will be shut down. This parameter can be provided as either a number or a reference, i.e. `{"Ref": "..."}`. If you use `fargate`, this must be an allowed value for Fargate tasks, and the default is 512. | Number/Ref | No | none
**reservation.softMemory** | The number of MB of RAM to reserve as a soft limit. Your worker container will be able to utilize more than this much RAM if it happens to be available on the host. This parameter can be provided as either a number or a reference, i.e. `{"Ref": "..."}`. If you use `fargate`, this option cannot be set. | Number/Ref | No | none
**reservation.cpu** | The number of CPU units to reserve for your worker container. This will only impact the placement of your container on an EC2 with sufficient CPU capacity, but will not limit your container's utilization. This parameter can be provided as either a number or a reference, i.e. `{"Ref": "..."}`. The minimum value is 128 if you do not use `fargate`. If you do use `fargate`, this must be an allowed value for Fargate tasks, and the default is 256. | Number/Ref | Yes | -
**privileged** | Give the container elevated privileges on the host container instance. If you use `fargate`, this option cannot be set. | Boolean/Ref | No | false
**writableFileSystem** | Give the container read-only access to the root file system. If you use `fargate`, this option cannot be set. | Boolean/Ref | No | false
...
**placementConstraints** | ECS service [placement constraints](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ecs-service-placementconstraint.html). If you use `fargate`, this option cannot be set. | Object[]/Ref | No | false
**placementStrategies** | ECS service [placement strategies](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ecs-service-placementstrategy.html). If you use `fargate`, this option cannot be set. | Object[]/Ref | No | false

### fargate mode explained
