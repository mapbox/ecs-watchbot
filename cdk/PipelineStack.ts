  ...
  constructor(scope: Construct, id: string, props: Props) {
    ...
    const linuxCodebuild = new aws_codebuild.PipelineProject(this, 'LinuxBundle', {
      ...
      buildSpec: BuildSpec.fromObject({
        ...
        phases: {
          ...
          build: {
            commands: ['node bin/watchbot-binary-generator']
          }
        }
      })
    });
    const alpineCodebuild = new aws_codebuild.PipelineProject(this, 'AlpineBundle', {
      ...
      buildSpec: BuildSpec.fromObject({
        ...
        phases: {
          install: {
            commands: ['apk add git', 'npm ci --production']
          },
          build: {
            commands: ['node bin/watchbot-binary-generator alpine']
          }
        }
      })
    });
    const testCodebuild = new aws_codebuild.PipelineProject(this, 'TestCodebuild', {
      ...
      buildSpec: BuildSpec.fromObject({
        ...
        phases: {
          pre_build: {
            commands: ['docker build -q -t ecs-watchbot -f test/Dockerfile ./']
          },
          build: {
            commands: ['docker run -t ecs-watchbot npm run test-container']
          }
        }
      })
    });
        ...
        main: new PolicyDocument({
          statements: [
            ...
            new PolicyStatement({
              ...
              resources: [linuxCodebuild.role?.roleArn || '', alpineCodebuild.role?.roleArn || '']
            })
          ]
        })
      }
