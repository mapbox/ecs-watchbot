 ...
 * must be an allowed value for Fargate tasks, and the default is 512.
 ...
 * number or a reference, i.e. `{"Ref": "..."}`. The minimum value is 128 if you
 * do not use `fargate`. If you do use `fargate`, this must be an allowed value
 * for Fargate tasks, and the default is 256.
...
module.exports = (options = {}) => {
  ...
  if (options.fargate) {
    if (!options.reservations) options.reservations = {};
    if (!options.reservation.memory) options.reservation.memory = 512;
    if (!options.reservation.cpu) options.reservation.cpu = 256;
