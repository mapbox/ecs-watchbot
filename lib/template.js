...
module.exports = (options = {}) => {
  ...
  options.reservation.memory =
    options.reservation.memory || cf.if(prefixed('CapacityIsNotEC2'), 512, cf.noValue);
  options.reservation.cpu =
    options.reservation.cpu || cf.if(prefixed('CapacityIsNotEC2'), 256, 128);
  ...
  const unpackEnv = (env, mountPoints) => {
    return Object.keys(env).reduce(
      ...
      [
        {
          Name: 'WorkTopic',
          Value: options.fifo ? undefined : cf.ref(prefixed('Topic'))
        },
        ...
        {
          Name: 'Volumes',
          Value: mountPoints.map((m) => m.ContainerPath).join(',')
        },
        { Name: 'Fifo', Value: options.fifo.toString() },
        {
          Name: 'structuredLogging',
          Value: options.structuredLogging.toString()
        }
      ]
    );
  };
  ...
  const mount = (mountInputs) => {
    ...
    if (typeof mountInputs === 'string') {
      mountInputs.split(',').forEach((mountStr) => {
        ...
        formatted.push(persistent ? mountStr.split(':')[1] : mountStr);
      });
    }
  ...
  if (options.notificationTopic && options.notificationEmail)
    throw new Error('Cannot provide both notificationTopic and notificationEmail.');
  if (!options.notificationTopic && !options.notificationEmail)
    throw new Error('Must provide either notificationTopic or notificationEmail.');
  ...
  if (options.fifo) {
    ...
    Resources[prefixed('DeadLetterQueue')].Properties.QueueName = cf.join([
      cf.stackName,
      '-',
      prefixed('DeadLetterQueue'),
      '.fifo'
    ]);
  }
  ...
  if (options.fifo) {
    ...
    Resources[prefixed('Queue')].Properties.QueueName = cf.join([
      cf.stackName,
      '-',
      prefixed('Queue'),
      '.fifo'
  ...
  if (options.reduce) {
    ...
    const tableArn = cf.join([
      'arn:aws:dynamodb:',
      cf.region,
      ':',
      cf.accountId,
      ':table/',
      cf.ref(prefixed('ProgressTable'))
    ]);
  ...
  Resources[prefixed('Task')] = {
    ...
    Properties: {
      ...
      ExecutionRoleArn: cf.if(
        prefixed('CapacityIsNotEC2'),
        cf.sub('arn:${AWS::Partition}:iam::${AWS::AccountId}:role/ecsTaskExecutionRole'),
        cf.noValue
      ),
      ContainerDefinitions: [
        {
          ...
          ReadonlyRootFilesystem: cf.if(
            prefixed('CapacityIsEC2'),
            !options.writableFilesystem,
            cf.noValue
          ),
          LogConfiguration: {
            LogDriver: 'awslogs',
  ...
  if (options.placementConstraints)
    Resources[prefixed('Service')].Properties.PlacementConstraints = options.placementConstraints;

  if (options.placementStrategies)
    Resources[prefixed('Service')].Properties.PlacementStrategies = options.placementStrategies;
  ...
  Resources[prefixed('ScaleUpTrigger')] = {
    ...
    Properties: {
      ...
      Namespace: 'AWS/SQS',
      Dimensions: [{ Name: 'QueueName', Value: cf.getAtt(prefixed('Queue'), 'QueueName') }],
  ...
  Resources[prefixed('AlarmMemoryUtilization')] = {
    ...
    Properties: {
      ...
      AlarmDescription: `https://github.com/mapbox/ecs-watchbot/blob/${options.watchbotVersion}/docs/alarms.md#memoryutilization`,
      Namespace: 'AWS/ECS',
  ...
  Resources[prefixed('TotalMessagesSchedule')] = {
    ...
    Properties: {
      ...
      Targets: [
        {
          Arn: cf.getAtt(prefixed('TotalMessagesLambda'), 'Arn'),
          Id: prefixed('TotalMessagesLambda')
        }
      ]
    }
  };
