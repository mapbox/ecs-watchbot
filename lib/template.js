...
module.exports = (options = {}) => {
  ...
  options.reservation.memory =
    options.reservation.memory ||
    cf.if(prefixed('CapacityIsNotEC2'), 512, cf.noValue);
  ...
  const Conditions = {
    ...
    [prefixed('CapacityIsFargateSpot')]: cf.equals(
      options.capacity,
      'FARGATE_SPOT'
    )
  };
  ...
  if (!options.notificationTopic && !options.notificationEmail)
    throw new Error(
      'Must provide either notificationTopic or notificationEmail.'
    );
  const notify =
    options.notificationTopic || cf.ref(prefixed('NotificationTopic'));

  if (options.notificationEmail)
    Resources[prefixed('NotificationTopic')] = {
      Type: 'AWS::SNS::Topic',
      Description:
        'Subscribe to this topic to receive emails when tasks fail or retry',
      Properties: {
        Subscription: [
          {
    ...
    Resources[prefixed('Dashboard')] = {
      ...
      Properties: {
        ...
        DashboardBody: cf.sub(dashboard, {
          ...
          WatchbotDeadLetterQueue: cf.getAtt(
            prefixed('DeadLetterQueue'),
            'QueueName'
          ),
  ...
  Resources[prefixed('Task')] = {
    ...
    Properties: {
      ...
      ExecutionRoleArn: cf.if(
        prefixed('CapacityIsNotEC2'),
        cf.sub(
          'arn:${AWS::Partition}:iam::${AWS::AccountId}:role/ecsTaskExecutionRole'
        ),
        cf.noValue
      ),
  ...
  Resources[prefixed('ScaleUp')] = {
    ...
    Properties: {
      ...
      StepScalingPolicyConfiguration: {
        ...
        StepAdjustments: [
          {
            ScalingAdjustment: cf.getAtt(
              prefixed('CustomScalingResource'),
              'ScalingAdjustment'
            ),
