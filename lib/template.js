 ...
 * @param {Number|ref} [options.cpuAlarmThreshold || options.alarmThreshold] - Use this parameter to control
 * the threshold that the service average CPU utilization must be over before triggering
 * an alarm. The default is 200, or 200% cpu utilization. This parameter
 * can be provided as either a number or a reference, i.e. `{"Ref": "..."}`.
 * @param {Number|ref} [options.cpuAlarmPeriods || options.alarmPeriods] - Use this parameter to control
 * the number of consecutive periods that the service average CPU utilization must be
 * over the threshold before triggering an alarm. You specify the number of 5-minute
 * periods before an alarm is triggered. The default is 4 periods, or 20 minutes.
 * Can be provided as either a number or a reference, i.e. `{"Ref": "..."}`.
 ...
 * either a number or a reference, i.e. `{"Ref": "..."}`.
 * @param {Number|ref} [options.messageAlarmThreshold=40] - Watchbot creates a
 ...
 * either a number or a reference, i.e. `{"Ref": "..."}`.
 * @param {Number|ref} [options.messageAlarmPeriods=24] - Use this parameter to control
...
module.exports = (options = {}) => {
  ...
  options = Object.assign(
    {
      ...
      errorThreshold: 10,
      messageAlarmPeriods: 24,
      messageAlarmThreshold: 40,
      cpuAlarmPeriods: 4,
      cpuAlarmThreshold: 200,
  ...
  Resources[prefixed('QueueSizeAlarm')] = {
    ...
    Properties: {
      ...
      EvaluationPeriods: options.alarmPeriods || options.messageAlarmPeriods,
      Statistic: 'Average',
      Threshold: options.alarmThreshold || options.messageAlarmThreshold,
      Period: '300',
      ComparisonOperator: 'GreaterThanThreshold',
      Namespace: 'AWS/SQS',
      MetricName: 'ApproximateNumberOfMessagesVisible',
  ...
  Resources[prefixed('AlarmCPUUtilization')] = {
    ...
    Properties: {
      ...
      ComparisonOperator: 'GreaterThanThreshold',
      Threshold: options.cpuAlarmThreshold,
      EvaluationPeriods: options.cpuAlarmPeriods,
      Statistic: 'Average',
      Period: 300,
      AlarmActions: [notify],
      Dimensions: [
