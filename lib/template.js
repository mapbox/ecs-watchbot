 ...
 * @param {string|ref} options.clusterRole - the name of an IAM role that EC2s
 * in the cluster will assume. Watchbot will graft permissions onto this role
 * that it requires in order to perform its work.
 ...
 * @param {string} [options.logAggregationFunction=''] - the ARN of the lambda function
 * to collect and send logs to sumologic
 ...
 * @param {number|ref} [options.reservation.memory=undefined] - the number of
...
module.exports = function(options) {
  ...
  if (options.webhook) webhook(prefixed, !!options.webhookKey, resources);
  ...
  if (options.logAggregationFunction) logAggregator(prefixed, resources, references);
...
function logAggregator(prefixed, resources) {
