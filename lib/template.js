 ...
 * @param {number|ref} [options.reservation.disk=undefined] - the amount of disk
 * space in GB to reserve for your worker container on a docker volume. This will
 * only impact the placement of your container on an EC2 with sufficient disk capacity,
 * but will not limit your container's utilization. This parameter can be provided as
 * either a number or a reference, i.e. `{"Ref": "..."}`. You must also provide a `.mounts`
 * option to configure the disk volume if you use a reservation.
...
module.exports = function(options) {
  ...
  if (options.reservation.disk)
    resources[prefixed('Worker')]
      .Properties
      .PlacementConstraints = [{
        Type: 'memberOf',
        Expression: cf.join(['attribute:availableDiskSpace >= ', options.reservation.disk])
      }];
