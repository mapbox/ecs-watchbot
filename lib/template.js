          ...
          exports.handler = function(event, context, callback) {
            ...
            try {
              const attrs = await sqs.getQueueAttributes({
                QueueUrl: ${'${QueueUrl}'}, // eslint-disable-line no-undef
                AttributeNames: ['ApproximateNumberOfMessagesNotVisible', 'ApproximateNumberOfMessages']
              }).promise();

              const metric = await cw.putMetricData({
                Namespace: 'Mapbox/ecs-watchbot',
                MetricData: [{
                  MetricName: 'TotalMessages',
                  Dimensions: [{ Name: 'QueueName', Value: ${'${QueueName}'} }], // eslint-disable-line no-undef
        ...
        PolicyDocument: {
          Statement: [
            ...
            },
            {
              ...
              Resource: cf.ref(prefixed('Queue'))
            }
          ]
        }
      }]
