 ...
 * @param {Object|null} [options.fargate=null] - if a fargate object is
 * provided, the Watchbot containers will run on Fargate capacity.
 * @param {Boolean|ref} [options.fargate.spot=false] - use Fargate Spot instances.
 * This is done by setting a capacity provider strategy on the Watchbot ECS
 * service, so the ECS cluster that you use must be configured to allow services
 * to use the `FARGATE_SPOT` capacity provider.
 * @param {Array|ref} [options.fargate.securityGroups] - the IDs of
 * security groups to associate with the Watchbot container. If you do not
 * specify a security group, the default security group for the VPC is used. You
 * can provide no more than 5.
 * @param {Array|ref} [options.fargate.subnets] - the IDs of subnets
 * to associate with the Watchbot container. If you do not specify a security
 * group, the default security group for the VPC is used. You can provide no
 * more than 16.
 ...
 * @param {Number|ref} [options.reservation.memory] - the number of MB of RAM to
 * reserve as a hard limit. If your worker container tries to utilize more than
 * this much RAM, it will be shut down. This parameter can be provided as either
 * a number or a reference, i.e. `{"Ref": "..."}`. If you use `fargate`, this
 * option must be set, and must be an allowed value for Fargate tasks.
 ...
 * If you use `fargate`, this option cannot be set.
 ...
 * your container's utilization. The minimum value is 128. This parameter can be
 * provided as either a number or a reference, i.e. `{"Ref": "..."}`. If you use
 * `fargate`, this option must be set, and must be an allowed value for Fargate
 * tasks.
 * @param {Boolean|ref} [options.privileged=false] - give the container elevated
 * privileges on the host container instance. If you use `fargate`, this option
 * cannot be set.
 * @param {Boolean|ref} [options.writableFilesystem=false] - give the container
 * read-only access to the root file system. If you use `fargate`, this option
 * cannot be set.
...
module.exports = (options = {}) => {
  ...
  if (options.fargate) {
    if (!options.reservation.memory || !options.reservation.cpu)
      throw new Error('reservation.memory and reservation.cpu are required when using Fargate');
    if (options.reservation.softMemory)
      throw new Error('reservation.softMemory is not compatible with Fargate');
    if (options.placementConstraints)
      throw new Error('placementConstraints is not compatible with Fargate');
    if (options.placementStrategies)
      throw new Error('placementStrategies is not compatible with Fargate');
    if (options.writableFilesystem)
      throw new Error('writableFilesystem is not compatible with Fargate');
  }
  ...
  if (options.fargate)
    Resources[prefixed('ExecutionRole')] = {
      Type: 'AWS::IAM::Role',
      Properties: {
        AssumeRolePolicyDocument: {
          Statement: [
        ...
        Policies: [
          {
            PolicyName: cf.join([cf.stackName, '-execution-role']),
            PolicyDocument: {
              Statement: [
                {
                  Action: [
                ...
                },
                {
                  ...
                  Resource: cf.getAtt('LogGroup', 'Arn')
                }
              ]
            }
          }
      ...
      ContainerDefinitions: [
        {
          ...
          ReadonlyRootFilesystem: options.fargate ? cf.noValue : !options.writableFilesystem,
          LogConfiguration: {
            LogDriver: 'awslogs',
            Options: {
  ...
  if (options.reservation.memory) {
    if (options.fargate) {
      Resources[prefixed('Task')].Properties.Memory = options.reservation.memory;
    } else {
      Resources[prefixed('Task')].Properties.ContainerDefinitions[0].Memory = options.reservation.memory;
    }
  }
  ...
  if (options.fargate) {
    Resources[prefixed('Task')].Properties.Cpu = options.reservation.cpu;
  } else if (!(options.reservation.cpu) || (!(options.reservation.cpu > 128) && (typeof options.reservation.cpu === 'number'))) {
    Resources[prefixed('Task')].Properties.ContainerDefinitions[0].Cpu = 128;
  }

  if (options.fargate)
    Object.assign(Resources[prefixed('Task')].Properties, {
      NetworkMode: 'awsvpc',
      RequiresCompatibilities: ['FARGATE'],
      ExecutionRoleArn: cf.getAtt(prefixed('ExecutionRole'), 'Arn')
    });
  ...
  if (options.fargate) {
    Resources[prefixed('Service')].Properties.NetworkConfiguration = {
      AwsvpcConfiguration: {
        SecurityGroups: options.fargate.securityGroups,
        Subnets: options.fargate.subnets
      }
    };
    if (options.fargate.spot) {
      Resources[prefixed('Service')].Properties.CapacityProviderStrategy = [
        { CapacityProvider: 'FARGATE_SPOT', Weight: 1 }
      ];
    } else {
      Resources[prefixed('Service')].Properties.LaunchType = 'FARGATE';
    }
  }
