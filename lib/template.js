 ...
 * @param {String} [options.capacityType='EC2'] - Can be 'FARGATE' or 'FARGATE_SPOT'`.
 * If no value is provided (it is falsy of  `{ "Ref": "AWS::NoValue" }`), the
 * Watchbot containers will run with the `EC2` launch type. If the value is
 * `FARGATE`, Watchbot will run with the `FARGATE` launch type. And if
 * `FARGATE_SPOT`, Watchbot will run on `FARGATE_SPOT` capacity provider. You
 * can only use `FARGATE_SPOT` on ECS clusters configured to allow the
 * `FARGATE_SPOT` capacity provider.
 * @param {Array|ref} [options.fargateSecurityGroups] - the IDs of security
 * groups to associate with the Fargate Watchbot container. This option is
 * ignored if `fargateMode` is not used. If you do not specify a security group
 * but use a `fargateMode`, the default security group for the VPC is used. You
 * can provide no more than 5.
 * @param {Array|ref} [options.fargateSubnets] - the IDs of subnets to associate
 * with the Fargate Watchbot container. This option is ignored if `fargateMode`
 * is not used. If you do not specify a subnets but use a `fargateMode` the
 * default subnets for the VPC are used. You can provide no more than
 * 16.
...
module.exports = (options = {}) => {
  ...
  options.reservation.memory = options.reservation.memory || cf.if(prefixed('CapacityIsNotEC2'), 512, cf.noValue);
  options.reservation.cpu = options.reservation.cpu
    ? Math.max((options.reservation.cpu || 0), 128)
    : cf.if(prefixed('CapacityIsNotEC2'), 256, Math.max((options.reservation.cpu || 0), 128));
  ...
  Resources[prefixed('ExecutionRole')] = {
    Type: 'AWS::IAM::Role',
    Condition: prefixed('CapacityIsNotEC2'),
    Properties: {
      AssumeRolePolicyDocument: {
        Statement: [
          {
  ...
  Resources[prefixed('Task')] = {
    ...
    Properties: {
      ...
      Memory: cf.if(prefixed('CapacityIsNotEC2'), options.reservation.memory, cf.noValue),
      Cpu: cf.if(prefixed('CapacityIsNotEC2'), options.reservation.cpu, cf.noValue),
      NetworkMode: cf.if(prefixed('CapacityIsNotEC2'), 'awsvpc', cf.noValue),
      RequiresCompatibilities: cf.if(prefixed('CapacityIsNotEC2'), ['FARGATE'], cf.noValue),
      ExecutionRoleArn: cf.if(prefixed('CapacityIsNotEC2'), cf.getAtt(prefixed('ExecutionRole'), 'Arn'), cf.noValue),
      ContainerDefinitions: [
        {
          ...
          Cpu: cf.if(prefixed('CapacityIsEC2'), options.reservation.cpu, cf.noValue),
          Memory: cf.if(prefixed('CapacityIsEC2'), options.reservation.memory, cf.noValue),
