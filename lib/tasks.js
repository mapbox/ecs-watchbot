 ...
 */
module.exports = function(cluster, taskDefinition, containerName, concurrency) {
  var ecs = new AWS.ECS({
  ...
  tasks.poll = function(callback) {
    ...
    ecs.describeTasks({ tasks: Object.keys(tasksInFlight) }, function(err, data) {
      if (err) return callback(err);
       ...
       */
      var taskStatus = data.tasks.filter(function(task) {
      ...
      taskStatus.free = concurrency - Object.keys(tasksInFlight).length;
