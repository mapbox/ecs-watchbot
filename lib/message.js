...
class Message {
  constructor(sqsMessage = {}, options = {}) {
    let valid = ['Body', 'MessageId', 'ReceiptHandle', 'Attributes'].reduce((valid, key) => {
      if (!valid) return false;
      return Object.prototype.hasOwnProperty.call(sqsMessage, key);
    }, true);

    if (!valid) throw new Error('Invalid SQS message object');

    valid = ['SentTimestamp', 'ApproximateFirstReceiveTimestamp', 'ApproximateReceiveCount'].reduce(
      (valid, key) => {
        if (!valid) return false;
        return Object.prototype.hasOwnProperty.call(sqsMessage.Attributes, key);
      },
      true
    );
    ...
    this.env = {
      ...
      Message: envMessage,
      SentTimestamp: new Date(Number(sqsMessage.Attributes.SentTimestamp)).toISOString(),
    ...
    this.logger = Logger.create(
      { type: 'watcher', structuredLogging: options.structuredLogging },
      this
    );
  }
