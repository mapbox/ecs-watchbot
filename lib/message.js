...
class Message {
  constructor(sqsMessage = {}, options = {}) {
    ...
    const messageGroupId = sqsMessage.Attributes.MessageGroupId;
    let envMessage;
    let envSubject;
    if (messageGroupId) {
      envMessage = sqsMessage.Body;
      envSubject = messageGroupId;
    } else {
      const snsMessage = JSON.parse(sqsMessage.Body);
      envMessage = snsMessage.Message;
      envSubject = snsMessage.Subject;
    }
