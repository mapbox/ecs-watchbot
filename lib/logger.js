...
class Logger {
  ...
  workerSuccess(results) {
    if (this.structuredLogging) {
      this.log(
        Object.assign({ level: INFO, worker_event: 'success' }, results)
      );
    } else {
  ...
  workerFailure(results) {
    if (this.structuredLogging) {
      this.log(
        Object.assign({ level: INFO, worker_event: 'failure' }, results)
      );
    } else {
  ...
  log(line) {
    if (this.structuredLogging) {
      ...
      if (this.message)
        minimum_structured_logline.watchbot_sqs_message_id = this.message.id;
      // If the line looks like JSON itself, then try to parse and merge it
      if (typeof line === 'string' && line[0] === '{') {
        try {
          process.stdout.write(
            JSON.stringify(
              Object.assign(minimum_structured_logline, JSON.parse(line))
            ) + '\n'
          );
        } catch (e) {
          ...
          process.stdout.write(
            JSON.stringify(
              Object.assign(minimum_structured_logline, { msg: line })
            ) + '\n'
          );
        }
