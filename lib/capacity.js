...
function run(argv, callback) {
  ...
  if (!argv.region || !argv.stack) {
    ...
    text += 'Example: worker-capacity --region us-east-1 --stack cats-api-staging';
...
function calculateRoom(resources, reservations) {
  var taskCapacity = 0;
  taskCapacity += resources.map((x) => {
    var workerCpu = x.find((e) => { return e.name === 'CPU'; }).integerValue / reservations.Cpu;
    var workerMemory = x.find((e) => { return e.name === 'MEMORY'; }).integerValue / reservations.Memory;
    return Math.min(workerCpu.toFixed(0), workerMemory.toFixed(0));
