  ...
  new Promise((resolve, reject) => {
    ...
    const child = child_process
      ...
      .on('exit', (code, signal) => {
        ...
        const response_duration = Math.max(
          0,
          now - new Date(options.env.SentTimestamp)
        );
    ...
    if (maxJobDuration > 0) {
      maxTimeout = setTimeout(() => {
        logger.log(
          `running killAll. duration has exceeded maxJobDuration. duration: ${
            Date.now() - start
          }`
        );
...
class Worker {
  constructor(message = {}, options = {}) {
    ...
    this.watcherLogger = Logger.create(
      { type: 'watcher', structuredLogging: options.structuredLogging },
      message
    );
    this.workerLogger = Logger.create(
      { type: 'worker', structuredLogging: options.structuredLogging },
      message
    );
  }
  ...
  async waitFor() {
    ...
    const heartbeatTimeout = setInterval(
      this.message.heartbeat.bind(this.message),
      120000
    );
