...
const child = async (command, options, logger, maxJobDuration) =>
  new Promise((resolve, reject) => {
    ...
    if (maxJobDuration > 0) {
      logger.log(`[worker] starting setTimeout. maxDuration: ${maxJobDuration}`);
      maxTimeout = setTimeout(() => {
        const duration = Date.now() - start;
        logger.log(`[worker] running child.kill(). maxDuration exceeded. duration: ${duration}`);
        child.kill();
      }, maxJobDuration * 1000);
    }
...
class Worker {
  ...
  async waitFor() {
    ...
    try {
      this.logger.log('Max Job Duration' + options.maxJobDuration);
      this.logger.log('this Max Job Duration' + options.maxJobDuration);
